  ...
  return (
    <div>
      <div className='max-w-7xl mx-auto px-4 py-8'>
        ...
        <p className='text-2xl font-medium mb-6 bg-gradient-to-r from-slate-600 to-slate-700 bg-clip-text text-transparent sm:hidden'>{t('dashboard.welcome')}</p>
        ...
        <div className='flex gap-4 flex-wrap'>
            <button onClick={()=> setShowCreateResume(true)} className='w-full bg-white sm:max-w-36 h-48 flex flex-col items-center justify-center rounded-lg gap-2 text-slate-600 border border-dashed border-slate-300 group hover:border-indigo-500 hover:shadow-lg transition-all duration-300 cursor-pointer'>
              ...
              <p className='text-sm group-hover:text-indigo-600 transition-all duration-300'>{t('dashboard.createResume')}</p>
            </button>
            <button onClick={()=> setShowUploadResume(true)} className='w-full bg-white sm:max-w-36 h-48 flex flex-col items-center justify-center rounded-lg gap-2 text-slate-600 border border-dashed border-slate-300 group hover:border-purple-500 hover:shadow-lg transition-all duration-300 cursor-pointer'>
              ...
              <p className='text-sm group-hover:text-purple-600 transition-all duration-300'>{t('dashboard.uploadExisting')}</p>
            </button>
            <button onClick={()=> setShowUploadAnnexe(true)} className='w-full bg-white sm:max-w-36 h-48 flex flex-col items-center justify-center rounded-lg gap-2 text-slate-600 border border-dashed border-slate-300 group hover:border-amber-500 hover:shadow-lg transition-all duration-300 cursor-pointer'>
              ...
              <p className='text-sm group-hover:text-amber-600 transition-all duration-300'>{t('dashboard.uploadAnnexes')}</p>
            </button>
            <button onClick={()=> setShowManageAnnexes(true)} className='w-full bg-white sm:max-w-36 h-48 flex flex-col items-center justify-center rounded-lg gap-2 text-slate-600 border border-dashed border-slate-300 group hover:border-teal-500 hover:shadow-lg transition-all duration-300 cursor-pointer'>
              ...
              <p className='text-sm group-hover:text-teal-600 transition-all duration-300'>{t('dashboard.manageAnnexes')}</p>
            </button>
        </div>
      ...
      <div className="grid grid-cols-2 sm:flex flex-wrap gap-4 ">
          ...
          return (
            <div key={index} className='relative w-full sm:max-w-36 h-48 rounded-lg border group hover:shadow-lg transition-all duration-300' style={{background: `linear-gradient(135deg, ${baseColor}10, ${baseColor}40)`, borderColor: baseColor + '40'}}>
              ...
              <p className='absolute bottom-1 left-0 right-0 text-[11px] text-center px-2' style={{ color: baseColor + '90' }}>
                 {t('dashboard.updatedOn')} {new Date(resume.updatedAt).toLocaleDateString()}
              </p>

              {/* Top right buttons - View, Annexes, Clone, Delete, Rename */}
              <div onClick={e=> e.stopPropagation()} className='absolute top-1 right-1 group-hover:flex items-center hidden'>
                <EyeIcon onClick={()=> navigate(`/app/preview-final/${resume._id}`)} className="size-7 p-1.5 hover:bg-white/50 rounded text-slate-700 transition-colors cursor-pointer" title={t('dashboard.viewFinal')}/>
                <PaperclipIcon onClick={()=> {setSelectedResumeId(resume._id); setShowAssignAnnexes(true)}} className="size-7 p-1.5 hover:bg-white/50 rounded text-slate-700 transition-colors cursor-pointer" title={t('dashboard.assignAnnexes')}/>
                <CopyIcon onClick={()=>cloneResume(resume._id)} className="size-7 p-1.5 hover:bg-white/50 rounded text-slate-700 transition-colors cursor-pointer" title={t('dashboard.clone')}/>
                <TrashIcon onClick={()=>deleteResume(resume._id)} className="size-7 p-1.5 hover:bg-white/50 rounded text-slate-700 transition-colors cursor-pointer" title={t('dashboard.delete')}/>
                <PencilIcon onClick={()=> {setEditResumeId(resume._id); setTitle(resume.title)}} className="size-7 p-1.5 hover:bg-white/50 rounded text-slate-700 transition-colors cursor-pointer" title={t('dashboard.rename')}/>
              </div>
            </div>
          )
        })}
        ...
        {showCreateResume && (
          <form onSubmit={createResume} onClick={()=> setShowCreateResume(false)} className='fixed inset-0 bg-black/70 backdrop-blur bg-opacity-50 z-10 flex items-center justify-center'>
            <div onClick={e => e.stopPropagation()} className='relative bg-slate-50 border shadow-md rounded-lg w-full max-w-sm p-6'>
              <h2 className='text-xl font-bold mb-4'>{t('dashboard.createResumeTitle')}</h2>
              <input onChange={(e)=>setTitle(e.target.value)} value={title} type="text" placeholder={t('dashboard.enterResumeTitle')} className='w-full px-4 py-2 mb-4 focus:border-green-600 ring-green-600' required/>

              <button className='w-full py-2 bg-green-600 text-white rounded hover:bg-green-700 transition-colors'>{t('dashboard.createResumeButton')}</button>
        ...
        {showUploadResume && (
          <form onSubmit={uploadResume} onClick={()=> setShowUploadResume(false)} className='fixed inset-0 bg-black/70 backdrop-blur bg-opacity-50 z-10 flex items-center justify-center'>
            <div onClick={e => e.stopPropagation()} className='relative bg-slate-50 border shadow-md rounded-lg w-full max-w-sm p-6'>
              <h2 className='text-xl font-bold mb-4'>{t('dashboard.uploadResumeTitle')}</h2>
              <input onChange={(e)=>setTitle(e.target.value)} value={title} type="text" placeholder={t('dashboard.enterResumeTitle')} className='w-full px-4 py-2 mb-4 focus:border-green-600 ring-green-600' required/>
                <div>
                  <label htmlFor="resume-input" className="block text-sm text-slate-700">
                    {t('dashboard.selectResumeFile')}
                    <div className='flex flex-col items-center justify-center gap-2 border group text-slate-400 border-slate-400 border-dashed rounded-md p-4 py-10 my-4 hover:border-green-500 hover:text-green-700 cursor-pointer transition-colors'>
                      {resume ? (
                        ...
                        <>
                          ...
                          <p>{t('dashboard.uploadResumePlaceholder')}</p>
                        </>
                      )}
                    </div>
                  </label>
              ...
              <button disabled={isLoading} className='w-full py-2 bg-green-600 text-white rounded hover:bg-green-700 transition-colors flex items-center justify-center gap-2'>
                ...
                {isLoading ? t('dashboard.uploading') : t('dashboard.uploadResumeButton')}
        ...
        {editResumeId && (
          <form onSubmit={editTitle} onClick={()=> setEditResumeId('')} className='fixed inset-0 bg-black/70 backdrop-blur bg-opacity-50 z-10 flex items-center justify-center'>
            <div onClick={e => e.stopPropagation()} className='relative bg-slate-50 border shadow-md rounded-lg w-full max-w-sm p-6'>
              <h2 className='text-xl font-bold mb-4'>{t('dashboard.editResumeTitle')}</h2>
              <input onChange={(e)=>setTitle(e.target.value)} value={title} type="text" placeholder={t('dashboard.enterResumeTitle')} className='w-full px-4 py-2 mb-4 focus:border-green-600 ring-green-600' required/>

              <button className='w-full py-2 bg-green-600 text-white rounded hover:bg-green-700 transition-colors'>{t('dashboard.updateButton')}</button>
        ...
        {showUploadAnnexe && (
          <form onSubmit={uploadAnnexe} onClick={()=> setShowUploadAnnexe(false)} className='fixed inset-0 bg-black/70 backdrop-blur bg-opacity-50 z-10 flex items-center justify-center'>
            <div onClick={e => e.stopPropagation()} className='relative bg-slate-50 border shadow-md rounded-lg w-full max-w-sm p-6'>
              <h2 className='text-xl font-bold mb-4'>{t('dashboard.uploadAnnexeTitle')}</h2>
              <input onChange={(e)=>setAnnexeTitle(e.target.value)} value={annexeTitle} type="text" placeholder={t('dashboard.enterAnnexeTitle')} className='w-full px-4 py-2 mb-4 focus:border-amber-600 ring-amber-600' required/>
                <div>
                  <label htmlFor="annexe-input" className="block text-sm text-slate-700">
                    {t('dashboard.selectPDFFile')}
                    <div className='flex flex-col items-center justify-center gap-2 border group text-slate-400 border-slate-400 border-dashed rounded-md p-4 py-10 my-4 hover:border-amber-500 hover:text-amber-700 cursor-pointer transition-colors'>
                      {annexeFile ? (
                        ...
                        <>
                          <UploadCloud className='size-14 stroke-1'/>
                          <p>{t('dashboard.uploadPDFAnnexe')}</p>
                        </>
                      )}
                    </div>
                  </label>
              ...
              <button disabled={isLoading} className='w-full py-2 bg-amber-600 text-white rounded hover:bg-amber-700 transition-colors flex items-center justify-center gap-2'>
                {isLoading && <LoaderCircleIcon className='animate-spin size-4 text-white'/>}
                {isLoading ? t('dashboard.uploading') : t('dashboard.uploadAnnexeButton')}
              </button>
