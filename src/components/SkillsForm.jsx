...
const SkillsForm = ({ data, onChange }) => {
  ...
  return (
    <div className='space-y-4'>
      <div>
        <h3 className='flex items-center gap-2 text-lg font-semibold text-gray-900'>{t('forms.skills.title')}</h3>
        <p className='text-sm text-gray-500'>{t('forms.skills.subtitle')}</p>
      </div>

      <div className="flex gap-2">
            <input type="text" placeholder={t('forms.skills.placeholder')} className='flex-1 px-3 py-2 text-sm'
            ...
            <button onClick={addSkill} disabled={!newSkill.trim} className='flex items-center gap-2 px-4 py-2 text-sm bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed'>
                <Plus className="size-4"/> {t('forms.skills.add')}
            </button>
      </div>
    ...
    (
        <div className='text-center py-6 text-gray-500'>
            ...
            <p className="text-sm">{t('forms.skills.getStarted')}</p>
        </div>
    )}

    <div className='bg-blue-50 p-3 rounded-lg'>
        <p className='text-sm text-blue-800'>{t('forms.skills.tip')}</p>
    </div>
    </div>
  )
}
